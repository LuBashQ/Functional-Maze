...
let createLetterDictionary (dim: int, letters: char[][][], char: char, color: Color) = 
    let pixelAlphabet = Dictionary<char,pixel[,]>()

    for letter in letters do
        let pixelLetter = Array2D.init dim dim (fun x y -> pixel.empty)
        for x = 1 to (dim - 1) do
            for y = 0 to (dim - 2) do
                if letter.[x].[y] = '*' then pixelLetter.SetValue (pixel.create (char, color),x,y)
        pixelAlphabet.Add(letter.[0].[0],pixelLetter)
    
    pixelAlphabet

let titleAlphbet (char: char, color: Color, isHighlight: bool, highlightColor: Color)= 
    let A = [|
        [|'A';'-';'-';'-';'-'|]
        [|'-';'*';'*';'*';'-'|]
        [|'*';'-';'-';'-';'*';|]
    ...
    createLetterDictionary (6,[|M;A;Z;E;Y;S;H;D;R;L;P|],'\219',(if isHighlight then highlightColor else color))